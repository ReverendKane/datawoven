...
from discovery_assistant.ui.info_text import PAIN_POINTS_INFO
from discovery_assistant.ui.widgets.screenshot_tool import ScreenshotTool
from discovery_assistant.storage import DatabaseSession, PainPoint, get_attachments_dir, get_screenshots_dir, FileManager, get_files_dir
from discovery_assistant.ui.widgets.draggable_table import DraggableTableWidget

_LOGGER = logging.getLogger("DISCOVERY.ui.tabs.pain_points_tab")
...
class PainPointItem:
    name: str
    impact: float = 0.5  # 0.0-1.0 scale (50% default)
    frequency: str = ""
    notes: str = ""
    ...
    def impact_percentage(self) -> int:
        """Convert impact to percentage for display"""
        return int(self.impact * 100)

    @property
    def impact_label(self) -> str:
        """Convert impact float to readable label"""
        if self.impact <= 0.2:
            return "Very Low"
        elif self.impact <= 0.4:
            return "Low"
        elif self.impact <= 0.6:
            return "Medium"
        elif self.impact <= 0.8:
            return "High"
        else:
            return "Very High"
...
# -------------------------
# Attachment dialog (reused from Processes)
# -------------------------
...
# -------------------------
# Enhanced attachment chip (same as Processes)
# -------------------------
...
# -------------------------
# Custom Frequency Radio Button Widget
# -------------------------

class FrequencyRadioGroup(QtWidgets.QWidget):
    """Custom radio button group for frequency selection"""
    selectionChanged = QtCore.Signal(str)

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setStyleSheet("background: transparent;")  # Remove gray background
        self._selected_frequency = "Daily"  # Default selection
        ...
        container.setObjectName("FrequencyRadioContainer")
        container.setStyleSheet("""
            QFrame#FrequencyRadioContainer {
                background: #F8FAFC;
                border: 1px solid #E2E8F0;
                border-radius: 8px;
                padding: 12px;
        ...
        # Inner layout for radio buttons
        ...
        frequencies = ["Randomly", "Daily", "Weekly", "Monthly", "Yearly"]

        for freq in frequencies:
            radio = QtWidgets.QRadioButton(freq)
            radio.setStyleSheet("""
                QRadioButton {
                    color: #374151;
                    font-size: 12px;
            ...
            if freq == "Daily":  # Set default selection
    ...
    def _on_selection_changed(self, button):
        self._selected_frequency = button.text()
        self.selectionChanged.emit(self._selected_frequency)

    def selected_frequency(self) -> str:
        return self._selected_frequency

    def set_frequency(self, frequency: str):
        self._selected_frequency = frequency
        for button in self.button_group.buttons():
            if button.text() == frequency:
                button.setChecked(True)
                break


# -------------------------
# Custom Impact Slider Widget
# -------------------------

class ImpactSlider(QtWidgets.QWidget):
    """Custom slider widget for impact rating (normalized 0.0-1.0 scale)"""
    valueChanged = QtCore.Signal(float)

    def __init__(self, parent=None):
        super().__init__(parent)
        self._value = 0.5  # Default to middle (50%)
        ...
        # Container frame for visual definition
        container = QtWidgets.QFrame()
        container.setObjectName("ImpactSliderContainer")
        container.setStyleSheet("""
            QFrame#ImpactSliderContainer {
                background: #F8FAFC;
                border: 1px solid #E2E8F0;
                border-radius: 8px;
                padding: 12px;
        ...
        # Inner layout for slider content
        layout = QtWidgets.QVBoxLayout(container)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setSpacing(8)

        # Labels row - positioned at exact intervals
        ...
        for i, label_text in enumerate(labels):
            ...
            label.setStyleSheet("font-size:11px; color:#6B7280; background:#FFFFFF;")
            label.setAlignment(QtCore.Qt.AlignCenter)

            if i == 0:
                # First label: left align
                labels_layout.addWidget(label, 0, QtCore.Qt.AlignLeft)
            elif i == len(labels) - 1:
                # Last label: right align
                labels_layout.addWidget(label, 0, QtCore.Qt.AlignRight)
            else:
                # Middle labels: add stretch before and after for centering
                labels_layout.addStretch()
                labels_layout.addWidget(label, 0, QtCore.Qt.AlignCenter)
                labels_layout.addStretch()

        # Slider - continuous range instead of discrete steps
        self.slider = QtWidgets.QSlider(QtCore.Qt.Horizontal)
        self.slider.setMinimum(0)
        self.slider.setMaximum(1000)  # High resolution for smooth movement
        self.slider.setValue(500)  # 50%
        # Remove ticks for smooth sliding
        ...
        self.slider.setStyleSheet("""
            ...
            QSlider::groove:horizontal {
                ...
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 #10B981, stop:0.25 #F59E0B, stop:0.5 #EF4444,
                    stop:0.75 #DC2626, stop:1 #991B1B);
                border-radius: 2px;
            }
    ...
    def _on_value_changed(self, value):
        self._value = value / 1000.0  # Convert to 0.0-1.0 range
    ...
    def setValue(self, value: float):
        ...
        self.slider.setValue(int(self._value * 1000))
        self._on_value_changed(int(self._value * 1000))


# -------------------------
# Main Pain Points Tab
# -------------------------
...
class PainPointsTab(QtWidgets.QWidget):
    ...
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None) -> None:
        ...
        scroller.setObjectName("PainPointsScroll")
        ...
        scroller.setStyleSheet("""
            QScrollArea#PainPointsScroll { background: transparent; border: none; }
        ...
        card.setObjectName("PainPointsCard")
        card.setStyleSheet("""
            QFrame#PainPointsCard {
                background:#FFFFFF;
                border:1px solid #E5E7EB;
                border-radius:12px;
            }
            QFrame#PainPointsCard QLabel { background: transparent; }
            QFrame#PainPointsCard QWidget#PainPointsForm { background: transparent; }
            QLineEdit, QTextEdit {
                background:#FFFFFF;
                border:1px solid #E2E8F0;
                border-radius:8px;
        ...
        section = InfoSection(
            title="Pain Points",
            subtitle="Identify areas where time is wasted or errors occur. Add attachments and screenshots for context.",
            info_html=PAIN_POINTS_INFO,
            icon_size_px=28,
            parent=card,
        )
        ...
        form_host.setObjectName("PainPointsForm")
        ...
        self.btn_add = QtWidgets.QPushButton("Add Pain Point")
        ...
        self.table.setObjectName("PainPointsTable")
        self.table.setHorizontalHeaderLabels(["Pain Name", "Impact", "Frequency", "Notes (preview)", "Attached Files"])
        ...
        self.btn_open_doc = QtWidgets.QPushButton("Edit Pain Point")
        ...
        self.btn_add.clicked.connect(self._add_pain_point)

        self.btn_open_doc.clicked.connect(self._load_pain_point_for_editing)
        self.btn_delete.clicked.connect(self._delete_selected)
        self.btn_cancel.clicked.connect(self._cancel_editing)

        self.table.itemDoubleClicked.connect(lambda *_: self._load_pain_point_for_editing())
    ...
    def _load_pain_points_data(self) -> None:
        """Load existing pain points from database with file validation."""
        try:
            with DatabaseSession() as session:
                pain_points = session.query(PainPoint).order_by(PainPoint.priority_rank.asc()).all()

                for db_pain_point in pain_points:
                    attachments = []
                    notes = db_pain_point.notes or ""
                    ...
                    # Convert database pain point to PainPointItem
                    pain_point_item = PainPointItem(
                        name=db_pain_point.pain_name,
                        impact=db_pain_point.impact or 0.5,
                        frequency=db_pain_point.frequency or "Daily",
                        notes=notes,
                        attachments=attachments
                    )

                    self._items.append(pain_point_item)
                    self._append_row(pain_point_item)

                _LOGGER.info(f"Loaded {len(pain_points)} pain points from database")

        except Exception as e:
            _LOGGER.error(f"Failed to load pain points data: {e}")

    def _save_pain_points_data(self) -> None:
        """Save current pain points list to database with file management."""
        try:
            with DatabaseSession() as session:
                # Clear existing pain points
                session.query(PainPoint).delete()

                # Save each pain point with file management
                for rank, item in enumerate(self._items, 1):
                    ...
                    if attachment_data:
                        import json
                        db_pain_point.notes += f"\n\n__ATTACHMENTS__:{json.dumps(attachment_data)}"

                _LOGGER.info(f"Saved {len(self._items)} pain points to database")

        except Exception as e:
            _LOGGER.error(f"Failed to save pain points data: {e}")

    def clear_fields(self) -> None:
        """Clear all pain points and clean up all files."""
        ...
        self.btn_add.setText("Add Pain Point")
        self.btn_cancel.setVisible(False)

        _LOGGER.info("Cleared all pain points and form fields")
    ...
    def _add_pain_point(self):
        """Add or update a pain point with database saving"""
        # Name can be empty; if so, generate a friendly default
        name = self.name_edit.text().strip()
        if not name:
            name = f"Untitled Pain Point — {QtCore.QDateTime.currentDateTime().toString('yyyy-MM-dd hh:mm')}"

        frequency = self.frequency_radio.selected_frequency()
        notes = self.notes_edit.toPlainText().strip()
    ...
    def _append_row(self, item: PainPointItem):
        """Add a row to the table"""
        row = self.table.rowCount()
        self.table.insertRow(row)
        self._populate_table_row(row, item)

    def _populate_table_row(self, row: int, item: PainPointItem):
        """Populate a table row with pain point item data"""
        ...
        self.table.setItem(row, 1, _cell(item.impact_label))
        self.table.setItem(row, 2, _cell(item.frequency))
        self.table.setItem(row, 3, _cell(preview))
    ...
    def _delete_selected(self):
        """Delete the selected pain point and clean up files"""
        idx = self._selected_row()
        if idx < 0:
            return

        if QtWidgets.QMessageBox.question(self, "Delete Pain Point",
                                          "Remove the selected pain point?") == QtWidgets.QMessageBox.Yes:
    ...
    def _load_pain_point_for_editing(self):
        """Load selected pain point into entry form and remove from table for editing"""
        idx = self._selected_row()
        if idx < 0:
            return
        ...
        if self._has_unsaved_changes():
            reply = QtWidgets.QMessageBox.question(
                self,
                "Unsaved Changes",
                "You have unsaved changes in the entry form. Load the selected pain point anyway?",
                QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No
            )
            if reply != QtWidgets.QMessageBox.Yes:
                return
        ...
        self.impact_slider.setValue(item.impact)
        self.frequency_radio.set_frequency(item.frequency)
        ...
        self._original_item = PainPointItem(
            name=item.name,
            impact=item.impact,
            frequency=item.frequency,
            notes=item.notes,
        ...
        self.btn_add.setText("Update Pain Point")
    ...
    def _has_unsaved_changes(self) -> bool:
        ...
        return (
            ...
            self.impact_slider.value() != 0.5 or
            self.frequency_radio.selected_frequency() != "Daily" or
    ...
    def _clear_entry_form(self):
        ...
        self.impact_slider.setValue(0.5)
        self.frequency_radio.set_frequency("Daily")
    ...
    def _table_context_menu(self, pos: QtCore.QPoint):
        ...
        act_open = menu.addAction("Edit Pain Point")
        act_del = menu.addAction("Delete")
        act = menu.exec(self.table.viewport().mapToGlobal(pos))
        if act == act_open:
            self._load_pain_point_for_editing()
        elif act == act_del:
    ...
    def _on_screenshot_saved(self, file_path: str, metadata: dict):
        ...
        if idx < 0:
            ...
            # Screenshot for existing pain point
